...
use ethereum_types::{Address, BigEndianHash, Bloom, H160, H256, U256};
use evm::backend::Log;
...
pub struct TransactionInfo {
    pub transaction: Option<Arc<SignedTransaction>>,
    pub post_state: H256,
    pub gas: U256,
    pub bloom: Bloom,
...
impl TransactionInfo {
    pub fn set_result(&mut self, res: Vec<u8>) {
...
impl UnverifiedTransaction {
    ...
    pub fn check_low_s(&self) -> Result<(), &'static str> {
        if !self.signature().is_low_s() {
            Err("UnverifiedTransaction.check_low_s: invalid signature")
        } else {
            Ok(())
        }
    }
    ...
    pub fn verify_basic(
        ...
        if self.is_unsigned() {
            return Err("verify_basic: unsigned");
        }
        if check_low_s {
            self.check_low_s()?;
        }
        match (self.chain_id(), chain_id) {
            ...
            _ => return Err("verify_basic: invalid chain id"),
        };
        Ok(())
    }
...
impl SignedTransaction {
    ...
    pub fn new(
        transaction: UnverifiedTransaction,
    ) -> Result<Self, parity_crypto::publickey::Error> {
    ...
    pub fn contract_address(&self) -> Option<H160> {
        match self.action {
            ...
            Action::Create => {
                ...
                stream.append(&nonce);
                let h: H160 = keccak_hash::keccak(&stream.out()).into();
                Some(h)
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn sender_test() {
        let bytes: Vec<u8> = hex::decode("f85f800182520894095e7baea6a6c7c4c2dfeb977efac326af552d870a801ba048b55bfa915ac795c431978d8a6a992b628d557da5ff759b307d495a36649353a0efffd310ac743f371de3b9f7f9cb56c0b28ad43601b4ab949f53faa07bd2c804").unwrap();
        ...
        assert_eq!(
            ...
            Address::from_str("0f65fe9276bc9a24ae7083ae28e2660ef72df99e").unwrap()
        );
    ...
    fn should_agree_with_vitalik() {
        let test_vector = |tx_data: &str, address: &'static str| {
            ...
            let bytes = rlp::decode(&data).expect("decoding tx data failed");
